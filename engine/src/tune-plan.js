 ...
 *   turn-budget   → phases.<id>.turn_budget  (canonical phase, and NOTHING already binds)
...
// A turn-budget rec fires on an OVERRUN, and an overrun is never evidence that the
// budget should be raised: the budget is self-counted, so overrunning it means it was
// not obeyed, not that it was too small. Auto-patching therefore applies to exactly one
// case — a phase where NOTHING currently binds (no manifest value, no descriptor value,
// no archetype-table entry), which is an inserted phase whose archetype the table does
// not cover. There a budget is genuinely absent, `from: null` is true, and proposing one
// is the only reading that adds a constraint rather than relaxing one. Every other
// overrun falls through to the advisory arm.
//
...
function turnBudgetPatchEligible(rec, base, effectiveBudgets) {
  if (rec.kind !== 'turn-budget') return false;
  if (!PHASE_NAMES.has(rec.phase)) return false;
  if (base.phases?.[rec.phase]?.turn_budget !== undefined) return false;
  // Fail-closed: a phase the caller could not resolve a budget for is NOT
  // eligible. An unreadable pipeline yields an empty map, and "unknown" must
  // never be read as "nothing binds" — that would auto-patch every overrun.
  if (!Object.hasOwn(effectiveBudgets ?? {}, rec.phase)) return false;
  return effectiveBudgets[rec.phase] === null;
}

// The knob is denominated in TOOL CALLS, which is what an agent can observe about
// itself; the miner's threshold is in billed turns. Deriving the proposed value from
// the rec's own measured tool calls keeps the unit of the number the same as the unit
// of the knob it is written into.
function proposedBudget(rec) {
  const observed = rec.evidence?.toolCalls;
  return Number.isInteger(observed) && observed > 0 ? observed : null;
}

function turnBudgetProposals(recs, base, effectiveBudgets) {
  const proposals = [];
  for (const rec of recs) {
    if (!turnBudgetPatchEligible(rec, base, effectiveBudgets)) continue;
    const to = proposedBudget(rec);
    if (to === null) continue;
    proposals.push({
      source: 'turn-budget',
      ...
      from: null,
      to,
      rationale: `budget ${to} tool calls for ${rec.phase}: ${rec.role} ran unbudgeted and used ${to} tool calls`,
...
function recAdvisory(rec, base, effectiveBudgets) {
  ...
  if (rec.kind === 'turn-budget') {
    if (turnBudgetPatchEligible(rec, base, effectiveBudgets)) return null;
...
function recAdvisories(recs, drift, base, effectiveBudgets) {
  ...
  return [...recs.map(rec => recAdvisory(rec, base, effectiveBudgets)).filter(Boolean), ...driftAdvisories];
}
 ...
 */
export function planTune({ report, memory, baseFrontmatter = {}, effectiveBudgets = {} }) {
  ...
  const autoProposals = [
    ...
    ...turnBudgetProposals(recs, baseFrontmatter, effectiveBudgets),
  ];
  const advisories = [
    ...recAdvisories(recs, drift, baseFrontmatter, effectiveBudgets),
