...
for (const descriptor of DESCRIPTORS) {
  ...
  // Copilot-binding fidelity: the Copilot binding reuses agent-mode assembly
  // outright — assembleContract has NO binding dimension. Passing a copilot
  // binding hint must be ignored (produce the same block as plain agent
  // mode); if a future change ever keys a variant off a binding, this trips.
  test(`Given a copilot binding hint on descriptor "${descriptor.id}", when assembled, then it is ignored and the block equals plain agent-mode assembly`, () => {
    const sut = assembleContract;

    const agentModeBlock = sut(descriptor, {}, FRAGMENTS, { execution: 'agent' });
    const copilotHintedBlock = sut(descriptor, {}, FRAGMENTS, { execution: 'agent', binding: 'copilot' });

    const diffs = diffLines(copilotHintedBlock, agentModeBlock);

    assert.equal(
      diffs.length,
      0,
      `Descriptor "${descriptor.id}": a binding hint must not introduce a variant — the copilot binding reuses agent-mode assembly, got ${JSON.stringify(diffs)}`,
    );
  });

  // Re-anchors the agent-vs-inline diff under the binding-fidelity concern
  // shared by every binding (opencode/copilot/pi): the two carve-out lines are
  // the ONLY binding-nameable slots. If a future change ever authors a
  // binding-specific variant (a third differing line), the diff set grows past
  // this confinement and trips the assertion. Binding-agnostic on purpose — no
  // binding hint is passed, since none of the per-binding hint tests above vary
  // this assertion; asserting it once per binding name killed no extra mutant.
  test(`Given descriptor "${descriptor.id}", when the carve-out set is checked, then the agent-vs-inline diff is confined to the two known carve-out lines`, () => {
    const sut = assembleContract;
    ...
    assert.equal(
      diffs.length,
      2,
      `Descriptor "${descriptor.id}": carve-out set must contain exactly the two known lines, got ${JSON.stringify(diffs)}`,
    );
    assert.ok(
