...
| run-record lines after `workspace` | flushed at the phase boundary only | walk step 7 (L307-322); spec "three write points" |
...
in `docs/contributing/specs/run-record.md`), R4 single-writer rule, ADR-363/371 (metrics bin;
transcripts identified by phase and window), ADR-369 (equiv weights are fixed constants),
ADR-361/365 (turn budget is agent-self-counted), ADR-217 (shim), ADR-223 (a hook predicate is
re-expressed per binding — this design ships the Claude binding only).
...
- **R1 — flush per line.** From §0 step 4 on, every run-record line is appended in the tool call
...
- **R2 — pre-workspace lines.** Lines produced between §0 step 4 and `workspace` land in a scratch
...
- **R4 — single writer unchanged.** Only the orchestrator's own tool calls append to the ledger,
  foreground or background.
- **R5 — reorient hook.** A plugin-registered `SessionStart` hook with matcher `compact` prints,
...
- **R6 — silent no-op.** The hook exits 0 with empty stdout when cwd is not in a git repo, when
  there is no run directory, when no pointer is bound to the session transcript, and when the bound
  pointer's ledger is gone. It never writes a file.
- **R7 — derivation is code.** `run-state` derives completed, in-flight and next phases and the
...
- **R9 — estimated compaction cost.** The miner reports per run the number of `compact_boundary`
...
- **R10 — docs.** `docs/guides/customizing.md` recommends an `autoCompactWindow` value and how to
...
gitignored, and their anonymised copy is R11. P1–P3 were pinned while writing this design.
...
<main>/.claude/craft-runs/                 <main> = dirname of git-common-dir (P1); gitignored
  <run-id>.pointer    "<run-key> <abs-ledger-path>"   open → move → close
  <run-id>.pre.md     scratch ledger, worktree strategy only, §0 step 4 → workspace
  <run-id>.delta.json memory delta, integrate step 3 → Done
<worktree>/.claude/craft-run-record.md      the ledger — path and line format unchanged
$TMPDIR/craft-review.XXXXXX/<dim>.c<N>.json normalised Finding[] (candidate 9)
```
...
### `scripts/run-ledger.sh` — the one write surface (candidate 8)
...
match: `open` printed the key, so it sits in the orchestrator's transcript (P2). A crashed run's
pointer is never in another session's transcript. A failed append follows the spec's existing
posture: surfaced in-session, and the run continues.
...
| `PHASE-START(<phase>): <iso8601>` | walk step 4, just before `contract-assemble` | `run-state`; `Done` `--since` |
| `PHASE-DONE(<phase>): <one-line outcome>` | walk step 7, every phase that ran (NO-OP phases included) | `run-state` (candidate 7) |
...
### Reorient hook — `hooks/reorient-after-compact.sh`
...
Algorithm: read the payload (stdin). Take `.cwd` (fallback `CLAUDE_PROJECT_DIR`) and `.transcript_path`
via `jq`. Run `run-ledger.sh locate --transcript <path>` from that cwd, resolving the script
relative to `BASH_SOURCE`. If nothing is printed, exit 0 silently. Otherwise print:
...
8,000, so it does not depend on any unpinned hook-output limit. Missing `jq` produces one stderr line and exit 0:
logged, never blocking a session in an unrelated repo. The hook never writes. The stale-pointer
...
### Sub-agents (D1, D2) — recommendation for candidate 4
...
long prompts). Every role's output except the reviewer's is a commit or a committed file. Reviewer output is covered by the respawn row above.
...
### Metrics — estimated compaction cost (D3, R9)
...
- `docs/guides/customizing.md` §4 gets a new use-pattern subsection, "Long sessions — auto-compaction (Claude Code)".
  It covers: the 1M default fires at 967k; the recommended value (candidate 2); per-session
  `claude --settings '{"autoCompactWindow":233000}'` or user settings; the value is read at launch
  only and inherited by every spawn; the plugin's reorient hook; craft never writes settings.
- `docs/contributing/specs/auto-compaction-poc-record.md` follows the `*-poc-record.md` shape (verdict,
  ...
  placeholders, Q6 scoring, Q7 table, spike 2 routes, P1–P3). No session id, transcript path, agent
...
| Situation | Behaviour |
|---|---|
| no craft run / non-git cwd / no `.claude/craft-runs/` | hook: exit 0, empty stdout |
...
| sub-agent compaction; manual `/compact` | same block (matcher `compact` covers both); the guard routes it |
...
| `hooks/reorient-after-compact.sh`, `hooks/hooks.json` | new / add `SessionStart` | as above |
...
| `docs/contributing/specs/run-record.md` | L9-19, L97-124, L126-136, L183, vocabulary L56-61 | the write rule, `craft-runs/`, tokens, a "Compaction survival" section |
...
|---|---|---|---|---|
| 1 | Hook activation | (a) always-on plugin hook; (b) opt-in manifest key; (c) opt-in settings snippet the user copies | (a) | The no-op path costs a `jq` parse and one `git rev-parse`, and prints nothing. Opt-in leaves the default unsafe for exactly the user who forgot to opt in. |
| 2 | Recommended `autoCompactWindow` | (a) `233000` (fires at 200k); (b) `283000` (fires at 250k); (c) document the formula, no value | (a) | Spike A's figure, and it keeps orchestrator turns ≤ 200k. Cost: planning and validation spawns will compact mid-task. (b) spares most planning spawns at +50k context on every orchestrator turn. Net token effect on spawns is extrapolated, not measured. |
| 3 | `PreCompact` instruction hook | (a) not now; (b) add now, same locate/guard, telling the summariser to keep run-id, ledger path, in-flight phase and landed commit hashes | (a) | Reinjection already scored 6/6 exact (Q6). Add (b) only if a live compaction shows drift the block does not cover (YAGNI). |
| 4 | D1 per-agent survival | (a) the block's sub-agent paragraph + reviewer respawn rule; (b) (a) + one `contracts/core.md` line so it also holds on non-Claude harnesses; (c) per-spawn state file | (a) | The hook reaches spawns anyway, and all outputs except reviewers' are commits. (b) is the portable variant, at the cost of a line in every spawn plus contract-test churn. (c) costs agent turns and adds in-tree sweep risk. |
| 5 | Active-run discovery + staleness | (a) pointer in `<main>/.claude/craft-runs/`, bound by run-key ∈ transcript; (b) scan `git worktree list` + probe ledgers, same binding; (c) pointer bound by `session_id` | (a) | (a) is O(1) and covers the pre-workspace window. (b) still needs the scratch for that window. (c) needs the orchestrator to learn its own session id, which is unpinned and would need a probe. |
| 6 | Scratch ledger location | (a) `<main>/.claude/craft-runs/<run-id>.pre.md`; (b) out-of-tree mktemp named by the pointer; (c) no scratch: re-run §0 after a pre-workspace compaction | (a) | (a) keeps one directory and is swept by `open`. (b) adds temp-cleanup semantics for no gain, since the pointer already writes the checkout. (c) is cheapest, because every pre-workspace line is re-derivable, but it contradicts brief scope 1. |
| 7 | Phase-completion token | (a) `PHASE-START` + `PHASE-DONE`; (b) `PHASE-DONE` only; (c) no token: artifacts + `GATE`/`NO-OP`/`auto-skip:` | (a) | Five phases are gate-less and `GATE` can be red before green. `PHASE-START` also persists the `--since` iso that `Done` needs and distinguishes mid-phase from not-started. (c) is brittle. |
| 8 | Ledger write surface | (a) `scripts/run-ledger.sh` shared by skill and hook; (b) raw `>>` per write point, with the hook re-implementing lookup; (c) a node engine bin | (a) | One root resolution for both callers (DRY), testable in a mktemp repo, and it keeps node out of the hook. (b) is today's approach and duplicates the lookup. |
| 9 | Review `Finding[]` location | (a) out-of-tree `mktemp -d`, path on the `FINDINGS` line; (b) `<worktree>/.claude/craft-review/`; (c) inline in the ledger | (a) | Follows validation's out-of-tree precedent (`contracts/producer.md` throwaway discipline): no sweep risk in repos that do not ignore `.claude/`. (c) floods the hook's tail. |
| 10 | Where the D3 estimate surfaces | (a) miner report only; (b) + a metrics-ledger column (format boundary); (c) + one advisory line from `emit-metrics` at `Done` | (a) | This is what the acceptance names. (b) breaks row comparability (ADR-369 rationale). (c) is a cheap later add. |
...
- **`test/hooks.test.js`**: `hooks.json` parses; `SessionStart[0].matcher === "compact"`; the command
  is the shimmed path; the script exists and is executable. Reorient cases, using payload fixtures
  with a generated cwd and transcript: non-git cwd → `''`, exit 0; no run dir → `''`; unbound
  pointer → `''`; torn-down ledger → `''`; bound from a **worktree** cwd → stdout's first line
  starts `craft reorient — if you are not the craft orchestrator`, contains `RESOLVE:` and only this
  run-id's lines, excludes another run-id's lines, and has length ≤ 8000 with a 500-line ledger of
  1,000-char lines.
...
- **Pinned-count updates**: `test/craft-root-shim.test.js` counts for each touched file (and a new
  `skills/implementation/SKILL.md` entry if it gains a shim); `test/living-corpus.test.js` EXPECTED
...
- **Live smoke (on demand, recorded in the poc-record, not CI)**: a throwaway session with
  ...
  sub-agent compaction received the guard. It closes the two unpinned assumptions: plugin versus
  settings hook parity, and key-in-transcript on a real main session.
