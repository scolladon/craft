...
(`skills/run/SKILL.md` §1c). Placement is a decision candidate, not a designer call.
...
2. **Item 6.** "An advisory manifest-lint warning on the *resolved* product" is not
   ...
   pipeline defaults). Surfaced as DC-13.
...
4. **Item 5 verb set.** `reset` cannot be banned without acknowledging that reset-on-red
   *is* a `git reset --hard` — performed by the runner, not by an agent. Handled in DC-12.

### Sizing corrections

- **Item 5 is smaller than framed** — one contract line plus its wording constraints and a
  presence test. No engine code.
- **Item 1 is larger than framed** — the ledger needs a file shape, a `.gitignore`
  re-include decision, a single-writer rule, a resume path, and an explicit restatement of
  the memory-save atomicity boundary.
- **Item 6 is larger than framed at manifest-lint** (a new warnings channel through
  ...
  pipeline-resolve** (one `records.push` in an existing function).
- **Item 3's hard part is not the lint, it is the detector** — "declares a file path" in a
  ...
  repetition.
- **Item 4 is roughly as framed**, with one addition: the DOD question needs a home
  decision because DOD.md is asserted on *every* change, and this question only applies to
  changes that propose a contract line.

## Requirements

- **R1 — the run record survives a context reset.** After each phase boundary, the run
  record's content for the current run is readable from disk without any session state. A
  new orchestrator instance, given the same brief, resolves the same run-id and reads back
  every line the run has written so far. Each record is one line, run-id-prefixed, so the
  ledger stays appendable and greppable; the final summary and the PR body take only the
  lines carrying this run's run-id.
- **R2 — the ledger is separate from the memory store.** No run-record content is written
  ...
  are independently readable and independently deletable.
- **R3 — memory-save atomicity is unchanged.** `save(...)` is still called exactly once,
  ...
  its pre-run content, while the ledger holds that run's partial lines.
...
- **R9 — plan-lint warns on cross-part context overlap.** Given a plan where two or more
  parts' `### Context` blocks declare a common file path, plan-lint emits one warning line
...
- **R12 — the DoD carries the matching authoring question**, in a home consistent with
  DOD.md's existing structure (frontmatter criterion ids, checklist-line-names-its-id, or
  the non-asserted reference block).
- **R13 — one new `contracts/core.md` line bans repo-wide git state mutation from any
  agent**, containing neither the substring `retrieval` (contracts-lint) nor the tokens
  `mutation`/`mutant` (source-hygiene Class A), and adding no new carve-out marker
  (contract-equivalence's exactly-two-differing-lines assertion holds).
- **R14 — the fan-out advisory is advisory.** It never changes a resolved value, never
  ...
  `dimensions.length × passes` reviewers.
...
**Shape.** Mirror `.claude/craft-metrics.md` exactly: one committed, append-only markdown
file at the repo ROOT (the worktree root — never `${CLAUDE_PLUGIN_ROOT}`, same rooting rule
as `memory.ref` in `skills/run/SKILL.md` §1c-mem), with a header line and one record per
line prefixed by the run-id:
...
**Write points.** Three, all orchestrator-owned:
...
3. **§Done.** Flush any residual lines, then run the memory save and the metrics append
   unchanged.
...
- The ledger flush **never** calls `save`, and `save` **never** reads or writes the ledger
  file. A mid-run block therefore leaves the store byte-identical (R3) while the ledger
  holds the partial run — which is precisely the respawn material R1 asks for.
...
- `delta` derivation at `Done` currently reads "the run-record-buffered observations". With
  the ledger on disk, the natural single source is the ledger's lines for this run-id
  (DC-4). That change is *source-of-truth only* — it does not move the save point, does not
  add a second save, and does not make the store incremental.
...
- **Cross-worktree:** each craft run works in its own git worktree, so each run has its own
  `.claude/` and its own ledger file. Interleaving two runs in one file is not a real case;
  the run-id prefix makes it survivable if it ever becomes one.
- **Run-id collision:** the run-id is the topic slug, so a genuine *re-run* of the same
  ...
  already carries repeat records for one id. It is correct for the case R1 exists to serve
  (resume) and over-broad for a deliberate re-run; the honest handling is to state it in
  the ledger's own doc rather than invent a second id nothing else uses.

**Gitignore.** `.gitignore:13` (`.claude/*`) excludes anything new under `.claude/`. If the
ledger is committed (DC-2), one re-include line joins lines 14-16.
...
(`skills/documentation/SKILL.md` step 3). With a shared append-only file, both consumers
must take **only the lines whose run-id prefix is this run's** — otherwise the first PR
after the ledger lands carries every prior run's outcomes. This is the one behavioural
change item 1 makes outside `skills/run/SKILL.md`, and it is a filter, not a reshaping: the
token vocabulary of the lines is untouched.
...
**Input contract.** The bin consumes a canonical `Finding[]` — i.e. it composes *after*
...
**Scope spec.** A single comma-joined string of `<file>:<start>-<end>` entries, mirroring
...
**Why a warning and not a failure.** Two implementer agents rebuilding the same mental
...
DC-7; the recommendation is advisory.
...
looks like a repo path. Detection strictness is DC-8.

**Where it lives.** Cross-part set intersection is awkward in awk and, more importantly,
`scripts/*.sh` is outside the mutation scope (`engine/stryker.conf.json` mutates
`engine/src/**` only). The house archetype for exactly this is the lint-bin-module
(`intention-lint`, `stub-lint`, `prose-lint`, `manifest-lint`): a thin `engine/bin` shim
over a pure `engine/src/<name>-main.js`. Implementation home is DC-9.

**Hard constraint either way:** the resolved `planning` gate string stays the literal
`plan-lint` (pinned by value at `engine/test/scenarios.test.js:210` and `:231`), so the
invocation name and path must not change, whatever the implementation home.
...
**Token.** The existing `PRUNE-CANDIDATE(<unit>): <rationale>` line is reused with no
change to its grammar (the brief's hard constraint, and the skill's own "defined here only"
claim stays true). Greppability of the two classes apart is DC-10.
...
Its home in DOD.md's structure is DC-11 — DOD.md is asserted on *every* craft change, and
this question only binds a change that proposes such a line.
...
**The line.** One new `contracts/core.md` line, in the same terse imperative register as
the other seven, naming the four verbs explicitly so it is greppable and unambiguous.
Candidate wordings and the reset-on-red question are DC-12.
...
A reviewer should expect both to keep working; neither needs an exemption clause in the
line itself (DC-12).
...
**Placement and threshold** are DC-13 and DC-14.

### Item 7 (conditional) — Frame 5 in `docs/guides/concepts.md`
...
frame produced that did not exist before it. Whether that clears the bar is the user's
call, not the designer's — DC-15.

If taken, the change is: one new `## Frame 5` section (narrative + mapping table in the
...
The six items are independent in mechanism. The natural seams and their couplings:

| Seam | Surfaces | Coupled to |
|---|---|---|
| A — on-disk run record | `skills/run/SKILL.md` §0.4 / walk step 7 / §Done; `.gitignore`; a run-record doc or spec page; `test/` presence tests | none of the others |
...
| C — plan-lint locality | `scripts/plan-lint.sh` (± `engine/bin/plan-lint.js` + `engine/src/plan-lint-main.js`); `templates/plan.md`; `engine/test`/`test` | shares the lint-bin archetype with B — do **not** share a part; different gates, different failure modes |
...
| F — fan-out advisory | `engine/src/resolve.js` *or* `engine/src/manifest{,-lint-main}.js`; `skills/review/SKILL.md`; `docs/guides/customizing.md`; `engine/test` | none, once DC-13 is decided |
| G — Frame 5 (conditional) | `docs/guides/concepts.md`; `README.md`; `docs/README.md` | consumes A/B/C as its mapping-table rows — land last |
...
`.gitignore` carries the re-include (if DC-2 = a); `skills/run/SKILL.md` §Done still states
`save` is called **once** and still states writes are buffered-and-flushed-once (an
anti-regression pin on R3 — the sentence must survive the edit); and the ledger path is not
`memory.ref`'s default (R2, a byte-level distinctness assertion). Edge matrix: absent file
(header written); present file (appended, header not duplicated); a run reaching `Done`
twice (documented as convergent under decay-merge, pinned as a prose invariant).
...
nothing); a Context block quoting a path in prose without backticks → no warning (DC-8's
boundary); two parts both naming the same *directory* span (`engine/src/`) → no warning;
...
'plan-lint'` must pass unchanged (R10). If DC-9 = (b), add
`engine/test/plan-lint-main.test.js` + `engine/test/plan-lint-bin.test.js` and register the
bin under the existing mutation scope.
...
appears in `skills/`); the rationale prefix, if DC-10 = (a), is pinned as a fixed string.
...
**Item 6 — fan-out advisory.** If DC-13 = (b): `engine/test/scenarios.test.js` /
`engine/test/resolve*.test.js` — a manifest with `passes: 1` and default dimensions emits
no advisory record; `passes: 3` with default dimensions emits exactly one advisory record
whose text names the product `12`; `passes: 3` with `dimensions: [code]` (product 3) emits
none; and — the R14 inertness proof — `effective[]`, `gateDecisions`, and every phase's
resolved `harness`/`reviewPlan` are deep-equal between a resolution that emits the advisory
and one that does not (the advisory may appear only in `record[]`). Boundary pair at the
DC-14 threshold: product == threshold → silent, product == threshold+1 → warns. If DC-13 = (a): add the
warnings-channel assertions to `test/manifest-lint.test.js` and confirm
`test/examples-lint.test.js` still sees exit 0 and `valid.` on every `examples/*/workflow.md`
(`examples/review-harness/workflow.md` resolves to 3 — below any candidate threshold, so it
must stay silent).

**Item 7 (if taken).** `test/living-corpus.test.js` unchanged (`docs/guides/concepts.md`
...
- **Per-technique output parsers.** Deliberately excluded by DC-5 and R6: shaping a
  technique's raw output into `Finding[]` belongs to the technique's declared
  `run`/`triage-procedure`, not to `engine/src`.
- **Changing `templates/plan.md`'s `### Context` schema.** DC-8 alternative (c) would make
  every existing plan non-conforming for an advisory check; if it is ever wanted, it is its
