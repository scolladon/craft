...
The stated working style is "a 4-dimension review after every code part," but the engine pipeline
models a single `review` phase over the whole change (per-dimension convergence). Nothing in the
skills states which is the canonical cadence, leaving the per-part interleave an undocumented
...
1. **Single phase canonical + working-style note** — the engine cadence is the one `review` phase;
   the per-part interleave is a documented session working-style (home: run/SKILL.md), cross-linked
   to the Parked-from-P8 walk/parallelism pass. No code change. *(designer's recommendation; chosen)*
2. **`review.cadence` harness knob** — model per-part as a run/harness knob (engine + data + walk).
3. **BACKLOG-note only** — record the decision, change no skill prose.

## Decision

The canonical cadence IS the single engine `review` phase. The per-part 4-dimension review is a
...
own a real per-part fan-out knob.

## Consequences

No code behaviour change; the engine's actual cadence is stated honestly. Modeling per-part review
