...
- Every part costs a full agent lifecycle — it must earn it.
- No standalone test-only parts: the alias bats fixture folds into part 1 (the wiring it
  guards); parts 2–4 are guarded by the **existing** suites (behavior-preserving), so their
  "RED" is "the suite that must stay green".

## Part 1 — wire `resolveAlias` into `validateManifest`; rename the `models` agent key
...
implement,docs,merge,review}` and `models.{part-implementer,reviewer,fallback}` validate — all
...
## Part 2 — rename the 9 skill dirs; delete the run/SKILL.md inverse-alias bridge
...
spawn ref in a body yet (the `mutation-triager` agent is still named that until part 3 — so
...
returns only the still-valid `craft:mutation-triager` (agent ref, part 3) and `craft:docs-writer`
...
## Part 3 — rename the `mutation-triager` agent; sweep stray old-name refs
...
`skills/validation/SKILL.md` (renamed in part 2) `craft:mutation-triager` → `craft:validation-triager`.
Sweep user-facing prose: `.claude-plugin/plugin.json` `description` (the "branch → design → ADR →
plan → implement-by-parts → review → refactor → mutation → docs → PR → merge" sequence → concern
...
- RED: none authored — `scripts/ci.sh` is the guardrail (the agent rename does not touch engine
  code; `manifest.test.js` part-1 tests already pin the `validation-triager` models key).
...
## Part 4 — ship the backlog doc template (ADR-014)
