...
| DC-D | Scope/sequencing of the three items (item 3 preconditioned on 1+2) | (a) **All three in this change**, two parts: Part 1 = Layer A; Part 2 = Layer B (`--harness` engine path). (b) **Items 1+2 only**, defer the flag to a follow-up. (c) **Item 3 only**, treat 1+2 as already-done since the descriptor carries them. | designer recommended **(a)** (part 1 originally framed as SKILL-only). | **Resolved to (a)** (ADR-099) — matches the recommendation: all three, two parts, A before B. **Per ADR-096, Part 1 / Layer A is no longer SKILL-only** — it carries the engine `reviewPlan` emission. Part order (A before B) still satisfies item 3's precondition. |
