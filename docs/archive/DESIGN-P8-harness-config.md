...
|---|---|---|
| R1 | `PHASE_FIELDS` in `manifest.js` is extended to the full honored set: `context`, `override`, `strategy`, `merge-flags`, `non-blocking-jobs`, `harness`, `execution`, `enabled`, `role`, `model` | `PHASE_FIELDS` expansion; one part, not incremental |
