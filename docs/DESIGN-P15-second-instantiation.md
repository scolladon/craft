...
| `planning` | `skills/planning/SKILL.md:20-21` — writes each slice gate as `gates.slice`, *else* "probe: the repo's test runner over touched files", *else* `gates.phase`. Gate `plan-lint` is craft-internal (awk over the plan artifact, `scripts/plan-lint.sh`). | **CLEAN** — `plan-lint` is repo-agnostic; gate is *probed*, not hardcoded |
| `implementation` | `skills/implementation/SKILL.md:10-14` — `gates.slice` (else repo test runner over touched files; else `gates.phase`) and `gates.phase` (**else: repo validate/check/test script; if nothing exists, REFUSE to run this phase**). | **CLEAN probe, but the REFUSE is the load-bearing SC5 edge** (see below) |
