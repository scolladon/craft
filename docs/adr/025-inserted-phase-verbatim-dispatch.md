...
The walk (`run/SKILL.md` step 1) invokes `craft:<phase.id>` and STOPs with "unknown phase id"
...
`procedure: craft:bench` (or a namespaced `acme:bench`) and a `role:`. So the walk must stop
assuming `craft:<id>`. The question is not *whether* to dispatch `phase.procedure` — that seam is
forced — but **how far P7 tests and what it promises** for the three flavors a `procedure:` can
take: ① an existing craft skill, ② a not-yet-existing craft skill, ③ a namespaced skill from a
derived plugin (SP2-proven cross-plugin dispatch; the `craft.extends:` registration surface is
explicitly P14).

## Options considered

1. **Craft-local now, namespaced→P14** — test ① end-to-end; document ③ as P7/P14 boundary.
...
3. **Craft-local only, no namespaced prose** — silent on ③. *Rejected:* leaves a mismatch — S7
...
dispatch line, `craft:bench` and `acme:bench` are identical strings; if the named skill/plugin is
not installed → loud STOP "procedure `<phase.procedure>` resolves to no installed skill". This is
*less* code than a `craft:`-only guard, not more — restricting would mean adding a check to remove
...
build the `craft.extends:` registration surface or an installed-derived-plugin end-to-end run —
those stay **P14**.

## Consequences

A user can insert a phase reusing an existing craft skill (e.g. a second `craft:review`) and it
runs today; a brand-new behavior needs a craft-local skill dir now, or a derived plugin once P14
