...
| Computational sensor | lints + gates (`design-lint`, `plan-lint`, `manifest-lint`, `docs-structure-lint`, `adr-lint`, the hygiene lints) via the Gate port `run(cmd)` | [../../scripts/](../../scripts/); [../contributing/specs/gate.md](../contributing/specs/gate.md) |
...
| Architecture-fitness harness | `architecture` phase (probe → run → triage → gate) + the intention port — two enumeration lanes: living pages reach `assert-fresh` and can go stale (`INTENTION-DRIFT` the sensor); governing ADRs reach `consult` only and never become freshness-guarded | [../../skills/architecture](../../skills/architecture); [../contributing/specs/intention.md](../contributing/specs/intention.md) |
...
| Outer loop — sampling sub-loop | decision candidates ratified in the `decisions` phase (ADRs); phase gates per round; the loop now closes — ratified ADRs feed back via the existing slot-1 intention prepend to `design`, `decisions` and `planning`, making "aligns with an existing ADR" an indexed read; a superseded decision emits `DECISION-REVERSAL(ADR-NNN)` into the run record, with `adr-lint` as the durable artifact-side guard | [../../skills/decisions](../../skills/decisions); [../../skills/design](../../skills/design); [../contributing/specs/intention.md](../contributing/specs/intention.md); [../contributing/specs/run-record.md](../contributing/specs/run-record.md) |
...
| External concept | craft mechanism (real) | Owning doc / key |
|---|---|---|
| Protect the orchestrator's own memory across a reset | the run record's on-disk ledger — an append-only `craft-runs/<run-id>.md` in the git common dir, orchestrator-only writer, appended per line as produced, run-local and never committed | [../contributing/specs/run-record.md](../contributing/specs/run-record.md); [../../skills/run](../../skills/run) |
...
| The orchestrator's tax | on-disk run-record ledger + validation boundary digest + plan-lint locality advisory + file ceiling | `craft-runs/<run-id>.md`; [../../skills/validation](../../skills/validation); `engine/bin/plan-lint.js` |
