...
describe('runAcceptanceProbe() — happy path (green trace)', () => {
  it('Given a green run-trace with gate-before-commit and mutations inside the throwaway, when probe runs, then passed is true', async () => {
    ...
    const copilotRunner = async () => GREEN_TRACE;

    const result = await sut({ copilotRunner, fsOps });
  ...
  it('Given a green run-trace, when probe runs, then evidence carries the expected keys', async () => {
    ...
    const REQUIRED_KEYS = ['targetPath', 'copilotVersion', 'model', 'portsExercised', 'phases'];
...
describe('runAcceptanceProbe() — never-commit-on-red structural assertion', () => {
  it('Given a run-trace where commit happened on a red gate, when probe runs, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => COMMIT_ON_RED_TRACE;
...
describe('runAcceptanceProbe() — mutations-inside-throwaway structural assertion', () => {
  it('Given a run-trace with a mutation outside the throwaway path, when probe runs, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => ESCAPED_MUTATION_TRACE;
...
describe('runAcceptanceProbe() — committed-artifact structural assertion', () => {
  it('Given a run-trace with no committed artifact, when probe runs, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => MISSING_ARTIFACT_TRACE;
...
describe('runAcceptanceProbe() — copilotRunner receives the correct invocation arguments', () => {
  it('Given the probe runs, when copilotRunner is called, then it receives phaseId implementation', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    let capturedArgs;
    const copilotRunner = async (args) => { capturedArgs = args; return GREEN_TRACE; };

    await sut({ copilotRunner, fsOps });

    assert.equal(capturedArgs.phaseId, 'implementation');
  });

  it('Given the probe runs, when copilotRunner is called, then it receives modelTier sonnet', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    let capturedArgs;
    const copilotRunner = async (args) => { capturedArgs = args; return GREEN_TRACE; };
  ...
  it('Given the probe runs, when copilotRunner is called, then it receives workingDir matching the throwaway path', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    let capturedArgs;
    const copilotRunner = async (args) => { capturedArgs = args; return GREEN_TRACE; };
...
describe('runAcceptanceProbe() — launch-args wiring (containment)', () => {
  it('Given the probe runs, when copilotRunner is called, then launchArgs contains --add-dir immediately followed by the throwaway path', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    let capturedArgs;
    const copilotRunner = async (args) => { capturedArgs = args; return GREEN_TRACE; };
    ...
    const addDirIndex = capturedArgs.launchArgs.indexOf('--add-dir');
    assert.notEqual(addDirIndex, -1);
    assert.equal(capturedArgs.launchArgs[addDirIndex + 1], THROWAWAY_PATH);
  });

  it('Given the probe runs, when copilotRunner is called, then launchArgs never contains --allow-all-paths', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    let capturedArgs;
    const copilotRunner = async (args) => { capturedArgs = args; return GREEN_TRACE; };
    ...
    assert.ok(!capturedArgs.launchArgs.includes('--allow-all-paths'));
  });
});
...
describe('runAcceptanceProbe() — default array fallbacks for optional trace fields', () => {
  it('Given a run-trace with mutatedPaths undefined, when probe runs, then passed is true (no mutations = safe)', async () => {
    ...
    const copilotRunner = async () => trace;
...
describe('runAcceptanceProbe() — throwaway-path trailing-slash normalisation', () => {
  ...
  it('Given a throwawayPath that already ends with a slash, when a mutatedPath equals the throwaway path without its trailing slash, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const trailingSlashPath = THROWAWAY_PATH + '/';
    const fsOps = makeFsOps(trailingSlashPath);
    const trace = {
...
describe('runAcceptanceProbe() — evidence shape and contents', () => {
  ...
  it('Given a green run-trace, when probe runs, then evidence.copilotVersion matches the trace copilotVersion', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => GREEN_TRACE;
    ...
    assert.equal(result.evidence.copilotVersion, GREEN_TRACE.copilotVersion);
  });
...
describe('runAcceptanceProbe() — gate-green-before-commit conjuncts isolated', () => {
  it('Given a run-trace with a red gate that ran before the commit, when probe runs, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => RED_BEFORE_COMMIT_TRACE;
  ...
  it('Given a run-trace with a green gate that ran after the commit, when probe runs, then passed is false', async () => {
    const sut = runAcceptanceProbe;
    const fsOps = makeFsOps(THROWAWAY_PATH);
    const copilotRunner = async () => GREEN_AFTER_COMMIT_TRACE;
