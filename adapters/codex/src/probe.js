 ...
 *
 * Drives ONE construction-bearing phase end-to-end through the Codex adapter
 * using an injected codexRunner / fsOps so all structural assertions are
 * unit-testable without a live Codex session — no test here, or anywhere in
 * this binding, spawns the real `codex` binary.
 *
 * Safety (state-mutating-probe rule): the entire probe runs inside a mktemp
 * throwaway repo — the launch argv built here (`buildLaunchArgs`) is the
 * posture that scopes a real Codex session's sandbox and containment root to
 * that throwaway, never the caller's worktree.
 *
 * Only three ports are exercised, deliberately fewer than copilot's four:
 * this binding binds no VCS port.
 */
...
function buildEvidence(trace, targetPath) {
  return {
    targetPath,
    codexVersion: trace.codexVersion,
    model: trace.model,
