...
| [`codex`](codex/) | local marketplace via `multi_agent_v1` subagent dispatch; the shared `skills/` load by **symlink, not by reference** — `plugin add` drops the out-of-tree reference (0 of 19 load without the symlinks, 19 of 19 with them) | **denying** PreToolUse hook (exit 2 genuinely blocks) + execpolicy defence-in-depth; trust is **scriptable** via `bin/trust-hook.js`, with `--check` as a read-only verification |
