 ...
 *
 * Drives ONE construction-bearing phase end-to-end through the opencode
 * adapter using an injected opencodeRunner / fsOps so all structural
 * assertions are unit-testable without a live opencode session.
 *
 * Safety (state-mutating-probe rule): the entire probe runs inside a
 * mktemp throwaway repo — opencode's tool-execute / exec surface and any
...
function buildEvidence(trace, targetPath) {
  return {
    targetPath,
    opencodeVersion: trace.opencodeVersion,
    model: trace.model,
...
export async function runAcceptanceProbe({ opencodeRunner, fsOps }) {
  ...
  const trace = await opencodeRunner({
    phaseId: PHASE_ID,
