#!/bin/bash
# forge — merge-phase cleanup. REFUSES while a mutation run-lock is alive.
#
# Lock protocol: the mutation phase writes <root>/.forge-mutation.lock containing
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --force)        FORCE=1; shift ;;
    *) echo "forge-teardown: unknown arg $1" >&2; exit 2 ;;
  esac
done

LOCK="$WT/.forge-mutation.lock"
if [ -f "$LOCK" ]; then
  read -r PID TS < "$LOCK" || true
  if [ -n "${PID:-}" ] && kill -0 "$PID" 2>/dev/null; then
    if [ "$FORCE" -eq 1 ]; then
      echo "forge-teardown: FORCED past live mutation run (pid=$PID since $TS) — run destroyed."
      rm -f "$LOCK"
    else
      echo "forge-teardown: REFUSED — mutation run alive (pid=$PID since $TS). Wait for it or pass --force." >&2
      exit 3
    fi
  else
    echo "forge-teardown: stale lock (pid=${PID:-?} dead) auto-cleared."
    rm -f "$LOCK"
  fi
fi
...
if [ -n "$PRE" ]; then
  ...
  echo "forge-teardown: pre-teardown script ran: $PRE"
fi

git -C "$MAIN" fetch --prune
if [ "$WT" != "$MAIN" ]; then
  git -C "$MAIN" worktree remove "$WT" --force
  echo "forge-teardown: worktree removed: $WT"
fi
if [ "$BRANCH" != "main" ] && [ "$BRANCH" != "master" ]; then
  git -C "$MAIN" branch -D "$BRANCH"
  echo "forge-teardown: local branch deleted: $BRANCH"
fi
