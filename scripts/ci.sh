...
run_prose_lint() {
  ...
  while IFS= read -r -d '' f; do
    [ -n "$f" ] || continue
    case "$f" in
      docs/contributing/adr/*|docs/contributing/design/*|docs/contributing/archive/*|docs/contributing/specs/*) ;;  # provenance/design docs necessarily quote ban-list words — advisory noise
