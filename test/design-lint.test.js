...
const MISSING_DC_FIXTURE = path.join(__dirname, 'fixtures', 'design-missing-dc.md');
...
test('design fixture missing Decision candidates exits 2', () => {
  let err;
  try {
    execFileSync('bash', [SCRIPT, MISSING_DC_FIXTURE], { encoding: 'utf8' });
    assert.fail('expected non-zero exit');
  } catch (e) {
    err = e;
  }
  assert.strictEqual(err.status, 2, 'expected exit status 2');
  assert.ok((err.stderr || '').includes('Decision candidates'), 'expected the failure to name the missing section');
});
