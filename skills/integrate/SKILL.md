...
3. **Consult `teardown`, then tear the worktree down.** Consult the `teardown` action
   separately before worktree teardown (per-verb granularity, ADR-126; see
   `docs/contributing/specs/policy.md`), append its `POLICY(...)` line, and:
   ```bash
   ...
   The run ledger lives in the git common dir, not the worktree, so it outlives the
   teardown: `Done` derives the memory delta from it afterwards
...
4. **Drift-baseline refresh offer:** when the merged change touched `skills/` or
   ...
   silently.
5. Deliver the final summary (PR URL, what shipped, record). `run-ledger.sh close` does
   not run here: it is the last action of the run skill's `Done`, after `save` and
   metrics have read the ledger.
