...
4. Check every drafted candidate against the denylist read in step 1. A candidate mapping
   to a core invariant, or to a non-overridable cross-phase invariant, is refused — drop it;
   it is never emitted.
5. Emit the surviving candidates as `PRUNE-CANDIDATE(<unit>): <rationale>` lines.
